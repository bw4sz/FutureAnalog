...
```{r}
load("../FutureAnalog_output/5_arcmins/sppXsite/mp85bi70.rda")
siteXsppF <- siteXspp
load("../FutureAnalog_output/5_arcmins/sppXsite/current.rda")
```
Start with mean traits

For each site (row) get the mean trait distance

```{r}

#container for site trait averages
out<-matrix(nrow=nrow(siteXspp),ncol=ncol(traits))
