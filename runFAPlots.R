...
ggsave("Figures/Novel_elevation_gam_RCP_20perc.png", width = 17, height = 9)
...
ggsave("Figures/Diss_elevation_gam_RCP_20perc.png", width = 17, height = 9)
...
ggplot(input.dat, aes(x=GCM, y=NoOfAnalogs)) + geom_boxplot() + 
  facet_grid(measure~comm.type)
input <- as.matrix(input.dat[,5:7])
input.diff <- list(data.frame(var = "RCP2.6 - RCP4.5", value = input[,1] - input[,2]),
                   data.frame(var = "RCP4.5 - RCP8.5", value = input[,2] - input[,3]),
...
for(com in unique(input.dat$comm.type)) {
  for(mes in unique(input.dat$measure)) {
    ...
    f <- friedman.test(as.matrix(dat[5:7]))
